...
switch ($method["choisir"]) {
    ...
    case "insert":
        ...
        // Grâce à la fonction bindvalue j'affecte chaque clé sa valeur.
        ...
        // J'envoie une réponse avec un success true, l'id de l'product que je viens d'insérer ainsi que l'image.
    ...
    case "update":
        ...
        if ($img) $img_stmt = ", product_picture = :img"; // S'il y a une image d'upload lors de la mise à jour je rajoute le bout de requête SQL correspondant.

        // J'écris une requete préparée de mise à jour du product à partir de son produit.
        $stmt = $bdd->prepare("UPDATE products SET product_name=:name, description=:desc, product_quantity=:quantity, product_price=:price, id_category=:id_category $img_stmt
        WHERE id_product = :id");
    ...
    case "delete":
        ...
        $stmt = $bdd->prepare("DELETE FROM products WHERE id = ? ");
